...
public class JSONReader
{
	...
	private Object parseValue() throws JSONParseException
	{
		...
		else if (token == JSONLex.JSONToken.JsString || token == JSONLex.JSONToken.JsInt ||
	...
	private Map parseObject() throws JSONParseException
	{
		...
		while (true) {
			...
			if (!isFirst) {
				...
				if (token != JSONLex.JSONToken.JsSeparator) {
					throw new JSONParseException("Object values not separated properly", null);
				}
				token = _lexer.getNextToken();
			}
			else {
